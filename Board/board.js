...
function renderArchivedCards() {
  ...
  allArchivedCards.forEach((card) => {
    ...
    cardElement.innerHTML = `
      <div class="archived-item__content">
        ...
        <div class="archived-item__actions">
          ...
          <button class="archived-item__delete" data-card-id="${card.id}" data-source="${source}" data-list-id="${listId || ''}">
            Xóa
          </button>
        </div>
      </div>
    ...
    const deleteBtn = cardElement.querySelector('.archived-item__delete');
    deleteBtn.addEventListener('click', () => {
      deleteCard(card.id, source, listId);
    });
...
function renderArchivedLists() {
  ...
  archivedLists.forEach((list) => {
    ...
    listElement.innerHTML = `
      <div class="archived-item__content">
        ...
        <div class="archived-item__actions">
          ...
          <button class="archived-item__delete" data-list-id="${list.id}">
            Xóa
          </button>
        </div>
      </div>
    ...
    const deleteBtn = listElement.querySelector('.archived-item__delete');
    deleteBtn.addEventListener('click', () => {
      deleteList(list.id);
    });
...
/**
 * Xóa card vĩnh viễn khỏi lưu trữ
 */
function deleteCard(cardId, source, listId) {
  // Xác nhận xóa
  const confirmMessage = `Bạn có chắc chắn muốn xóa vĩnh viễn thẻ này?\n\nHành động này không thể hoàn tác.`;
  ...
  if (source === 'inbox') {
    cardIndex = cardsInbox.findIndex((c) => c.id === cardId);
    if (cardIndex !== -1) {
      card = cardsInbox[cardIndex];
      cardsInbox.splice(cardIndex, 1);
      localStorage.setItem('cardsInbox', JSON.stringify(cardsInbox));
    }
  } else {
    cardIndex = cards.findIndex((c) => c.id === cardId);
    if (cardIndex !== -1) {
  ...
  if (!card) {
    console.warn("[CARD] Không tìm thấy thẻ để xóa:", { cardId, source });
    return;
  }
  
  console.log("[CARD] Đã xóa vĩnh viễn thẻ:", {
    card: { ...card },
    source,
    listId,
    cardIndex
...
/**
 * Xóa list vĩnh viễn khỏi lưu trữ và tất cả card có listId đó
 */
function deleteList(listId) {
  const list = lists.find((l) => l.id === listId);
  if (!list) {
    console.warn("[LIST] Không tìm thấy danh sách để xóa:", listId);
    return;
  }
  
  // Đếm số card sẽ bị xóa
  const cardsInList = cards.filter((card) => card.listId === listId);
  const cardsInListCount = cardsInList.length;
  
  // Xác nhận xóa
  const confirmMessage = `Bạn có chắc chắn muốn xóa vĩnh viễn danh sách "${list.title}"?\n\nTất cả ${cardsInListCount} thẻ trong danh sách này cũng sẽ bị xóa vĩnh viễn.\n\nHành động này không thể hoàn tác.`;
  if (!confirm(confirmMessage)) {
    return;
  }
  
  // Xóa tất cả card có listId này
  const cardsToDelete = cards.filter((card) => card.listId === listId);
  ...
  console.log("[LIST] Đã xóa vĩnh viễn danh sách:", {
    list: { ...list },
    deletedCardsCount: cardsToDelete.length,
    deletedCards: cardsToDelete.map(c => ({ id: c.id, title: c.title }))
  });
...
function setupCardModal() {
  ...
  // Setup nút lưu trữ card
  const storeCardBtn = document.getElementById("store-card-btn");
  storeCardBtn?.addEventListener("click", handleStoreCardFromModal);
...
/**
 * Lưu trữ card từ modal
 */
function handleStoreCardFromModal() {
  if (!currentCardContext) {
    console.warn("[CARD] Không có context để lưu trữ card");
    return;
  }

  const cardRef = findCardReference(currentCardContext);
  if (!cardRef) {
    console.warn("[CARD] Không tìm thấy thẻ để lưu trữ", currentCardContext);
    return;
  }

  // Đặt status thành "done" nếu chưa phải (để có thể lưu trữ)
  if (cardRef.status !== "done") {
    cardRef.status = "done";
  }
  ...
  console.log("[CARD] Đã lưu trữ thẻ từ modal:", {
    card: { ...cardRef },
    source: currentCardContext.source,
    listId: currentCardContext.listId,
  });
